...
- **导出标识符**（类型、函数、方法、常量）一律有 Go doc 注释，以名字开头（如 `// Service 内容业务逻辑。`）；每个包有 `doc.go` 或包注释。
- **结构体字段逐个加行尾注释**说明用途/单位/取值含义，含 DTO、model、config、请求/响应结构。仅当字段名已自解释（如 `ID`、`CreatedAt`）可省略，但同一结构体内有字段需注释时建议全部补齐以对齐。字段注释用 `gofmt` 对齐。
- 错误经统一错误码（`internal/pkg/errcode`）+ `response` 包返回，不裸传 error。
